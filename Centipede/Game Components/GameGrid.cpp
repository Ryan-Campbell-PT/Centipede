...
void GameGrid::GetCenterGridPosition(sf::Vector2f & pos)
{
	...
	GetInstance()->GetCenterXPosition(pos);
	GetInstance()->GetCenterYPosition(pos);
}
...
void GameGrid::GetCenterXPosition(sf::Vector2f & pos)
{
	pos.x = static_cast<float>(static_cast<int>(pos.x / SPRITE_SIZE) * SPRITE_SIZE + (SPRITE_SIZE / 2));
}
...
{
	const auto tmpX = (v.x / SPRITE_SIZE);
	const auto tmpY = (v.y / SPRITE_SIZE);
